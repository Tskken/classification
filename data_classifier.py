"""Feature extraction methods and harness code for data classification.
...
def basic_feature_extractor_digit(datum):
    """Return a set of pixel features indicating white (0) or black (1)."""
...
def basic_feature_extractor_face(datum):
    """Return a set of pixel features indicating whether each pixel is an edge.

    Returned features encoded as 1 for edges, 0 for no edges.
    """
    features = util.Counter()
...
def basic_feature_extractor_pacman(state):
    """Returm basic features for pacman.
...
def enhanced_feature_extractor_pacman(state):
    """Feature extraction playground for pacman.
    ...
    for action in state.get_legal_actions():
        features[action] =\
            util.Counter(features[action],
                         **enhanced_pacman_features(state, action))
    return features, state.get_legal_actions()


def enhanced_pacman_features(state, action):
    """Return enhanced features for pacman.
    ...
    test_variable = [util.manhattan_distance(pacman_position, fpos)
                     for fpos in successor.get_food().as_list()]
    if len(test_variable) == 0:
        closest_food = 0
    else:
        closest_food = min(test_variable)

    closest_ghost = min([util.manhattan_distance(pacman_position, gpos)
                         for gpos in successor.get_ghost_positions()])
...
def analysis(classifier, guesses, test_labels, test_data, raw_test_data,
             print_image):
    """Code to help you analyze your results.
    ...
    - guesses is the list of labels predicted by your classifier on the
        test set
    - test_labels is the list of true labels
    - test_data is the list of training datapoints
        (as util.Counter of features)
...
class ImagePrinter:
    ...
    def print_image(self, pixels, ax=None):
        """Print the image either in text or plot.

        This will serve as a helper function to the analysis function you
        write.
...
USAGE_STRING = """
  ...
  EXAMPLES:   (1) python data_classifier.py
                  - trains the default most_frequent classifier on the digit
                    dataset using the default 100 training examples and
                    then test the classifier on test data
              (2) python data_classifier.py -c naive_bayes -d digits -t 1000 \
-f -o -1 3 -2 6 -k 2.5
                  - would run the naive Bayes classifier on 1000 training
                    examples using the enhanced_feature_extractor_digits
                    function to get the features on the faces dataset,
                    would use the smoothing parameter equals to 2.5,
                    would test the classifier on the test data and performs an
                    odds ratio analysis with label1=3 vs. label2=6
                 """


def learning_rate_callback(option, opt, value, parser):
    """Set learning rates (used as a callback from option parser)."""
    setattr(parser.values, option.dest, [float(s) for s in value.split(',')])


def read_command(argv):
    """Process the command used to run from the command line."""
    from optparse import OptionParser
    parser = OptionParser(USAGE_STRING)

    parser.add_option('-c', '--classifier',
                      help=default('The type of classifier'),
                      choices=['most_frequent', 'nb', 'naive_bayes',
                               'perceptron', 'perceptron_numpy', 'ann',
    ...
    parser.add_option('-n', '--num_weights',
                      help=default("Num Weights to Print "
                                   "(when --weights enabled), default: 100"),
                      default=100, type="int")

    parser.add_option('-k', '--smoothing',
                      help=default("Smoothing parameter "
                                   "(ignored when using --autotune)"),
                      type="float", default=2.0)

    parser.add_option('-a', '--autotune',
                      help=default("Whether to automatically tune "
    ...
    parser.add_option('-l', '--learning_rates',
                      help=default("Learning rates to use for gradient "
                                   "descent, can be a comma separated "
                                   "list or single value"),
                      default=[0.2], type="str", action='callback',
    ...
    if options.training <= 0:
        print("Training set size should be a positive integer "
              "(you provided: %d)" % options.training)
        print(USAGE_STRING)
        sys.exit(2)

    if options.smoothing <= 0:
        print("Please provide a positive number for smoothing "
              "(you provided: %f)" % options.smoothing)
        print(USAGE_STRING)
        sys.exit(2)

    if options.odds:
        if ((options.label1 not in legal_labels or
             options.label2 not in legal_labels)):
            print("Didn't provide a legal labels for the odds ratio:"
                  " (%d,%d)" % (options.label1, options.label2))
            print(USAGE_STRING)
            sys.exit(2)
...
MAP_AGENT_TO_PATH_OF_SAVED_GAMES = {
    'FoodAgent': ('pacmandata/food_training.pkl',
                  'pacmandata/food_validation.pkl',
                  'pacmandata/food_test.pkl'),
    'StopAgent': ('pacmandata/stop_training.pkl',
                  'pacmandata/stop_validation.pkl',
                  'pacmandata/stop_test.pkl'),
    'SuicideAgent': ('pacmandata/suicide_training.pkl',
                     'pacmandata/suicide_validation.pkl',
    ...
    'ContestAgent': ('pacmandata/contest_training.pkl',
                     'pacmandata/contest_validation.pkl',
...
def run_classifier(args, options):
    ...
    if(options.data == "pacman"):
        ...
        training_data, validation_data, test_data =\
            MAP_AGENT_TO_PATH_OF_SAVED_GAMES.get(agent_to_clone,
                                                 (None, None, None))
        ...
        raw_training_data, training_labels =\
            samples.load_pacman_data(training_data, num_training)
        raw_validation_data, validation_labels =\
            samples.load_pacman_data(validation_data, num_test)
        raw_test_data, test_labels =\
            samples.load_pacman_data(test_data, num_test)

    else:
        raw_training_data =\
            samples.load_data_file("digitdata/trainingimages",
                                   num_training, DIGIT_DATUM_WIDTH,
                                   DIGIT_DATUM_HEIGHT)
        training_labels =\
            samples.load_labels_file("digitdata/traininglabels", num_training)

        raw_validation_data =\
            samples.load_data_file("digitdata/validationimages",
                                   num_test, DIGIT_DATUM_WIDTH,
                                   DIGIT_DATUM_HEIGHT)

        validation_labels =\
            samples.load_labels_file("digitdata/validationlabels", num_test)

        raw_test_data =\
            samples.load_data_file("digitdata/testimages", num_test,
                                   DIGIT_DATUM_WIDTH, DIGIT_DATUM_HEIGHT)

        test_labels = samples.load_labels_file("digitdata/testlabels",
                                               num_test)
    ...
    print("Training...")
    classifier.train(training_data, training_labels, validation_data,
                     validation_labels)
    ...
    print(str(correct), ("correct out of " + str(len(validation_labels)) +
                         " (%.1f%%).") %
          (100.0 * correct / len(validation_labels)))

    print("Testing...")
    guesses = classifier.classify(test_data)
    correct = [guesses[i] == test_labels[i]
               for i in range(len(test_labels))].count(True)

    print(str(correct), ("correct out of " + str(len(test_labels)) +
                         " (%.1f%%).") % (100.0 * correct / len(test_labels)))

    analysis(classifier, guesses, test_labels, test_data,
             raw_test_data, print_image)

    # do odds ratio computation if specified at command line
    if ((options.odds and
         (options.classifier == "naive_bayes" or (options.classifier == "nb"))
         )):
        ...
        if(options.classifier == "naive_bayes" or options.classifier == "nb"):
            string3 = ("=== Features with highest odd ratio of label "
                       "%d over label %d ===" % (label1, label2))
        else:
            string3 = ("=== Features for which weight"
                       "(label %d)-weight(label %d) is biggest ===" %
                       (label1, label2))

        print(string3)
        print_image(features_odds)

    if ((options.weights and ((options.classifier in
                              ["perceptron_numpy", "ann"])))):

        for i, l in enumerate(classifier.legal_labels):
            features_weights = classifier.find_high_weight_features(
                                    l, options.num_weights)
            print("=== Plotting Features with high weight for label"
                  " %d ===" % l)
