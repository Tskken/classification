...
class NaiveBayesClassifier(classification_method.ClassificationMethod):
    ...
    def __init__(self, legal_labels):
        """Initialize the classifier given the legal labels."""
        self.legal_labels = legal_labels
        self.type = "naivebayes"
        #  this is the smoothing parameter, ** use it in your train method **
        self.k = 1

        #  Look at this flag to decide whether to choose k automatically
        #  ** use this in your train method **
        self.automatic_tuning = False

    def set_smoothing(self, k):
        """Set the the smoothing parameter to k.

        This is used by the main method to change the smoothing parameter, and
    ...
    def train(self, training_data, training_labels, validation_data,
        ...
        self.features = list(set([f for datum in training_data
                                  for f in list(datum.keys())]))
        ...
        self.train_and_tune(training_data, training_labels, validation_data,
                            validation_labels, kgrid)

    def train_and_tune(self, training_data, training_labels, validation_data,
                       validation_labels, kgrid):
        ...
        training_data and validation_data are lists of feature Counters.
        The corresponding label lists contain the correct label for each datum.

        To get the list of all possible features or labels, use self.features
        and self.legal_labels.
        """
        # best accuracy so far on validation set
        best_accuracy_count = -1
        ...
        # Conditional probability of feature feat being 1
        # indexed by (feat, label)
        common_conditional_prob = util.Counter()

        # how many time I have seen feature feat with label y
        ...
        for k in kgrid:  # Smoothing parameter tuning loop!
            ...
            for label in self.legal_labels:
                for feat in self.features:
                    ...
                    # 2 because both value 0 and 1 are smoothed
            ...
            accuracy_count = [predictions[i] == validation_labels[i]
                              for i in range(len(validation_labels))
                              ].count(True)

            print("Performance on validation set for k=%f: (%.1f%%)" %
                  (k, 100.0*accuracy_count/len(validation_labels)))
    ...
    def calculate_log_joint_probabilities(self, datum):
        """Return the log-joint distribution over legal labels and the datum.
        ...
        for label in self.legal_labels:
            ...
            for feat, value in list(datum.items()):
                if value > 0:
                    log_joint[label] +=\
                        math.log(self.conditional_prob[feat, label])
                else:
                    log_joint[label] +=\
                        math.log(1-self.conditional_prob[feat, label])

        return log_joint

    def find_high_odds_features(self, label1, label2):
        """Return the 100 best features for the odds ratio.

        Odds ratio is   P(feature=1 | label1)/P(feature=1 | label2)

        Note: you may find 'self.features' a useful way to loop through
        all possible features
        """
        features_odds = []

        for feat in self.features:
            features_odds.append((self.conditional_prob[feat, label1] /
                                  self.conditional_prob[feat, label2], feat))
        features_odds.sort()
