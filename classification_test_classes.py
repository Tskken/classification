"""Test classes for the classification assignment.
...
def read_digit_data(training_size=100, test_size=100):
    """Read digit data and split up into training, validation, test sets."""
    rootdata = 'digitdata/'
    # loading digits data
    raw_training_data = samples.load_data_file(
        rootdata + 'trainingimages', training_size,
        DIGIT_DATUM_WIDTH, DIGIT_DATUM_HEIGHT)

    training_labels = samples.load_labels_file(
        rootdata + "traininglabels", training_size)

    raw_validation_data = samples.load_data_file(
        rootdata + "validationimages", TEST_SET_SIZE,
        DIGIT_DATUM_WIDTH, DIGIT_DATUM_HEIGHT)

    validation_labels = samples.load_labels_file(
        rootdata + "validationlabels", TEST_SET_SIZE)

    raw_test_data = samples.load_data_file(
        "digitdata/testimages", test_size,
        DIGIT_DATUM_WIDTH, DIGIT_DATUM_HEIGHT)
    ...
    return (training_data, training_labels, validation_data, validation_labels,
            raw_training_data, raw_validation_data, test_data, test_labels,
            raw_test_data)


def read_suicide_data(training_size=100, test_size=100):
    """Read suicide data and split up into training, validation, test sets."""
    rootdata = 'pacmandata'
    raw_training_data, training_labels =\
        samples.load_pacman_data(rootdata + '/suicide_training.pkl',
                                 training_size)

    raw_validation_data, validation_labels =\
        samples.load_pacman_data(rootdata + '/suicide_validation.pkl',
                                 test_size)
    raw_test_data, test_labels =\
        samples.load_pacman_data(rootdata + '/suicide_test.pkl', test_size)
    training_data = []
    validation_data = []
    test_data = []
    return (training_data, training_labels, validation_data, validation_labels,
...
def read_contest_data(training_size=100, test_size=100):
    """Read contest data and split up into training, validation, test sets."""
    rootdata = 'pacmandata'
    raw_training_data, training_labels =\
        samples.load_pacman_data(rootdata + '/contest_training.pkl',
                                 training_size)

    raw_validation_data, validation_labels =\
        samples.load_pacman_data(rootdata + '/contest_validation.pkl',
                                 test_size)

    raw_test_data, test_labels =\
        samples.load_pacman_data(rootdata + '/contest_test.pkl', test_size)
    training_data = []
    validation_data = []
    test_data = []
    return (training_data, training_labels, validation_data, validation_labels,
...
def tiny_data_set_peceptron_and_mira():
    """Create a tiny data set for perceptron and mira."""
    def count(m, b, h):
        c = util.Counter()
        c['m'] = m
        c['b'] = b
...
def get_accuracy(
        data, classifier,
        feature_function=data_classifier.basic_feature_extractor_digit):
    """Compute the test accuracy of classifier on data."""
    ...
    serialized_guesses = ", ".join([str(guesses[i])
                                    for i in range(len(test_labels))])
    print(str(correct),
          ("correct out of " + str(len(test_labels)) + " (%.1f%%).") % (acc))
    return acc, serialized_guesses


class GradeClassifierTest(test_classes.TestCase):
    """Class for grading classifiers."""

    def __init__(self, question, test_dict):
        """Initialize with given question and test_dict."""
        ...
        self.accuracy_thresholds =\
            [int(s) for s in
             test_dict.get('accuracy_thresholds', '').split()]

        self.exact_output = test_dict['exact_output'].lower() == "true"

        self.automatic_tuning = (
            test_dict['automatic_tuning'].lower() == "true"
            if 'automatic_tuning' in test_dict else None)

        self.max_iterations = (int(test_dict['max_iterations'])
                               if 'max_iterations' in test_dict else None)

        self.feature_function = (test_dict['feature_function']
                                 if 'feature_function' in test_dict
                                 else 'basic_feature_extractor_digit')

        self.time_scale = (float(test_dict['time_scale'])
                           if 'time_scale' in test_dict else 1.0)

        self.compare_time = (test_dict['compare_time'] if 'compare_time'
                             in test_dict else None)

        self.record_time = (test_dict['record_time'] if 'record_time'
                            in test_dict else None)

        self.learning_rates = ([float(s) for s in
                                test_dict['learning_rates'].split()]
                               if 'learning_rates' in test_dict else None)

        self.max_points = len(self.accuracy_thresholds) * self.accuracy_scale

    def grade_classifier(self, module_dict):
        """Grade the student code."""
        ...
        classifier_class = getattr(module_dict[self.classifier_module],
        ...
        return get_accuracy(data, classifier,
        ...
        if self.compare_time or self.record_time:
            ...
            if self.compare_time:
                with open(self.compare_time) as f:
                    ...
                    self.add_message(
                        "Original took: %2.4fs, optimized took %2.4fs" %
...
class MultipleChoiceTest(test_classes.TestCase):
    """A multiple choice test (answer saved as encrypted value."""
    ...
    def execute(self, grades, module_dict, solution_dict):
        """Run the test."""
        student_solution = str(getattr(module_dict['answers'],
                               self.question)())
        encrypted_solution =\
            sha1(student_solution.strip().lower().encode("utf-8")).hexdigest()
